...
if __name__ == '__main__':
    ...
    with strategy.scope():
        ...
        for layer in mod_with_fc_raw.layers[-20:]:
            if not isinstance(layer, tf.keras.layers.BatchNormalization):
                layer.trainable = True
        # Last conv2d layer. This we woriginally trained.
        # Remember to un-comment this if we need to revert to what worked.
        ...
        efficient_net_b_7_transferable_base_model =\
            tf.keras.Model(inputs=mod_with_fc_raw.layers[0].input, 
                            outputs=mod_with_fc_raw.layers[-2].output)
