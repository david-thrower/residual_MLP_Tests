...
6. API:
    ...
    3. To build a ResidualMLP model:
        ...
        5.  Arguments:
            ...
            15. final_activation - The activatinn fucntion e.g. tf.keras.activations.sigmoid - [no parentheses after it] if you are doing binary classification. None if you are doing regression. The defaults is tf.keras.activations.softmax sunce most problems are multi class classification.
            16. loss - The loss that is appropriate for your problem. E.g. if we are doign simple linear regression, the default chice is usually tf.keras.losses.MeanSquaredError(), but tf.keras.losses.Huber() and tf.keras.losses.MeanAbsoluteError() are fair game. The default is: tf.keras.losses.CategoricalCrossentropy(from_logits=False), since most problems are multi-class classification.
