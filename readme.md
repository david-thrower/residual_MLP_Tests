...
6. API:
    ...
    3. To build a ResidualMLP model:
        ...
        6.  Arguments:
            ...
            16. loss - The loss that is appropriate for your problem. E.g. if we are doign simple linear regression, the default choice is usually tf.keras.losses.MeanSquaredError(), but tf.keras.losses.Huber() and tf.keras.losses.MeanAbsoluteError() are fair game in regression cases and one may performe better than the other. For binary, you might set this as tf.keras.losses.BinaryCrossentropy. The default is: tf.keras.losses.CategoricalCrossentropy(from_logits=False), since most problems are multi-class classification.
